...
function categorizeRepos(repos) {
  ...
  for (const repo of repos) {
    ...
    if (topics.includes('web') || topics.includes('webapp') || name.includes('app') || desc.includes('web app') || lang === 'typescript' || lang === 'javascript') {
      category = 'Web Apps';
    } else if (topics.includes('library') || topics.includes('package') || name.includes('lib') || desc.includes('library')) {
      category = 'Libraries';
    } else if (topics.includes('cli') || name.includes('cli') || desc.includes('command line')) {
      category = 'CLI Tools';
  ...
  const projectCards = Object.entries(categories).map(([cat, repos]) => {
    ...
    const cards = repos.map(r => `
      <div class="card">
        ...
        <div class="meta">
          <span class="lang">${r.language || 'Unknown'}</span>
          <span class="stars">★ ${r.stars}</span>
        </div>
        <a href="${r.url && r.url.startsWith('https://') ? r.url : '#'}" target="_blank" class="link">View Repository →</a>
      </div>
    `).join('');
...
function handleRequest(req, res, categories, username) {
  ...
  if (url.pathname === '/api/refresh') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ status: 'ok', message: 'Restart server to refresh' }));
    return;
  }
  
  res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
  res.end(generateHTML(categories, username));
}
...
export default async function runApp({ port }) {
  ...
  const server = http.createServer((req, res) => {
    handleRequest(req, res, categories, username);
  });
